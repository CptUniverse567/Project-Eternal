...
## Pack 01 — Outer Reaches (decision date: 2026-08-17)

- **Version bump**: `0.2.0` (versionCode 2). First content pack per the roadmap.
- **Regions (data-first):** added **Cindervale** (tier 2, `region:cindervale`, price 1.6, regional rare **emberglass** via `RareYieldConfig`) and **Frostreach** (tier 3, `region:frostreach`, price 2.0, regional rare **frostvein** + a **hazard** `hazardPerAction`). Deepened **Stormreach** with 4 new nodes + 4 monsters/elites. No existing region/quest/token changed — the new regions slot parallel to the existing chain (`q_emberreach_ash → q_cindervale`, `q_stormreach_herald → q_frostreach`) and neither unlock requires content inside itself.
- **Regional mechanics**: `RegionDefinition` gained `hazardPerAction` and `rareYield` (a single, legible `RareYieldConfig`), applied in `ActivityEngine.advanceGathering` (player) and `RetainerEngine` (retainers). Frostreach's hazard chips HP per action, floored at the combat retreat floor (20% max HP) so it is bounded/offline-safe; tested offline for determinism and floor.
- **Enhancement depth**: `EnhancementTable.alternateMaterialPerAttempt` lets the ADVANCED band (+16..+30) accept **frostvein** or the existing voidforged crystal; `GameIntent.Enhance.useAlternateMaterial` + `Engine.enhance` consume the chosen material; the gear tab renders a second enhance button. No probability/formula changes. New checkpoint quests `q_advance_begin` (+5), `q_frostvein_enhance` (+16), `q_advance_mid` (+20), `q_advance_near` (+30) ladder toward the unchanged +31 gate.
- **New crafting skill**: **Jewelry** (accessories: Ember Ring, Ashveil Amulet, Frostvein Ring, Icebound Amulet) — added only because the ACCESSORY slots needed a crafting home; leather reuses Drying/Armorcraft (no skill bloat).
- **Gear slots**: GLOVES, BOOTS, ACCESSORY_1/2 now populated by the Cindervale (T2.5) and Frostforge (T3.5) families; all enhanceable; durability/repair/enhancement work through the existing generic gear UI.
- **Workers**: new `RetainerSpecialization.PROSPECTOR` and `FORGER` (both **gathering-only**; retainer processing explicitly deferred to Pack 02). Recruits Pella (FORGER, 950, `region:cindervale`) and Runa (PROSPECTOR, 1400, `region:frostreach`); FORGER gains a MINE/SPECIAL speed affinity, PROSPECTOR a find-chance (luck) affinity; new `Vein Sense` trait.
- **Tool bonuses data-driven**: `ItemDefinition.gatheringSkill` + `gatheringSpeedMultiplier` replaced the hardcoded pickaxe check in `Rates` (also finally wires the fishing rod's advertised +25%).
- **Consumables data-driven**: `ItemDefinition.healAmount` + `useConsumable` now read healing from content (existing heal values preserved).
- **UI (additive)**: region cards show the rare-find rule and a hazard warning; node cards warn on hazardous regions; gear detail line lists the alternate enhancement material.
- **Deadlock audit — Refining (confirmed & fixed)**: `refine_refined` (Refine Iron) required Refining 1 with **no level-0 Refining XP source** — a fresh player could not reach Refining 1, blocking the entire Cindervale gate (q_cindervale needs Steel). Fixed by setting `refine_refined.skillLevelRequired = 0` (content fix). Same class of fix for **Engineering** (`craft_machinery_part` 1→0) and **Jewelry** (`craft_ring_ember` 2→0). A ContentIntegrity guardrail now asserts every PROCESSING/CRAFTING skill has a level-0 recipe.
- **Tests**: `ContentIntegrityTest` extended to 15 (new regions/nodes/monsters/quests/enhancement alternate material/resource sinks/deadlock guardrail); new `OuterReachesTest` (23) covers region gates (no circular deps), resource chains, bosses/loot, workers, enhancement alternate material + checkpoints, hazard, regional rares, offline determinism, and the refining→steel→Cindervale end-to-end path. Full JVM suite **166 tests / 0 failures**; instrumented `EternalCriticalPathTest` **3/3** on API-36; `assembleDebug` green.
- **On-device playtest (fresh save)**: verified fresh start → mining → grind → smelt → **Refine Iron now starts at Refining 0** (the fix) → refined iron → craft/equip Bronze Sword → wolves (offline) → Guardian quest; durability break + repair-vs-reforge decision exercised; Engineering card renders "lv 0 required". Coordinate-tap fragility (blind taps) mis-crafted a Hemp Rope once — a playtest/UX note, not a game bug.
- **Latent (non-blocking)**: `recipe:sword_steel` and `recipe:craft_shield_oak` are defined but never granted by any quest (steel sword / oak shield recipes unreachable). No quest requires them, so this is class E (future-pack dependency), documented not fixed.

## Post-playtest UX pass — Inventory & Enhancement (decision date: 2026-08-17)

- **Equipment selling (new)**: Equipment instances (`GameState.equipmentItems`) are now sellable for Marks via `GameIntent.SellEquipment(uid)` and `SellAllEquipment(defId)`. The engine validates: instance exists, not equipped (equipped items are **protected**), `sellPrice > 0`. Price is data-driven from `ItemDefinition.sellPrice` × regional modifier — no special formula; all current and future equipment works automatically. Broken/damaged equipment can be sold. Bulk: `SellAllEquipment` sells every **unequipped** instance of a defId, preserving equipped copies.
- **Sell All for stackables**: `GameIntent.SellAll(defId)` sells the entire stack of any stackable item (materials, consumables). Simple transaction; no confirmation dialog for small stacks per the existing project UX convention.
- **Gear inventory Sell button**: An **on-equipment Sell button** now appears on every unequipped piece in the `PackItemRow`. Equipped items (in the `EquippedSection`) do **not** show a Sell button — equipped protection is enforced at both UI (button absent) and engine (sell/equipUid guard) levels. No "favorite/lock" concept was invented; the sole rule is "equipped = protected."
- **Market Sell Equipment section**: A dedicated "Sell Equipment" section groups unequipped equipment by `defId` with per-type Sell 1 + Sell All buttons. This keeps the existing material/consumable Sell section unaffected.
- **Enhancement UI restructured**: `PackItemRow` was rewritten into a vertically stacked layout (no horizontally squeezed buttons): header → durability → Equip (full-width) → Repair (full-width) → `EnhanceBlock` (full-width header info + vertically stacked Actions). The block is now `EnhanceBlock` (a standalone composable) that scales naturally with future enhancement options (Oil, Ward, Full Negation, Alternate material all full-width per row). No enhancement mechanics changed.
- **Schema unchanged**: All changes operate on existing serialized data (`ItemInstance` has no new fields). No schema version bump needed.
- **Latent**: `Recipe.kt` still has a duplicate `cook_roast` entry (resolved by `associateBy`; harmless). Deferred to future cleanup.
- **Tests**: `EquipmentSellTest` (10 tests) covers single/bulk equipment sale, equipped protection, damaged/broken behavior, Pack 01 gear, exact Marks, empty inventory, persistence. Full JVM suite: **176 tests / 0 failures**. Instrumented: **3/3**. `assembleDebug` green.
